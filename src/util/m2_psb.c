...
typedef enum {
    ...
    PSB_ITYPE_TRUE = 0x02,
    PSB_ITYPE_FALSE = 0x03,

    PSB_ITYPE_INTEGER_0 = 0x04,
    PSB_ITYPE_INTEGER_8 = 0x05,
    PSB_ITYPE_INTEGER_16 = 0x06,
    PSB_ITYPE_INTEGER_24 = 0x07,
    PSB_ITYPE_INTEGER_32 = 0x08,
    PSB_ITYPE_INTEGER_40 = 0x09, // assumed, decomp does same as 32b due to int cast (compiler over-optimization?)
    PSB_ITYPE_INTEGER_48 = 0x0A, // same
    PSB_ITYPE_INTEGER_56 = 0x0B,
    PSB_ITYPE_INTEGER_64 = 0x0C,

    PSB_ITYPE_LIST_8  = 0x0D,
    PSB_ITYPE_LIST_16 = 0x0E,
    PSB_ITYPE_LIST_24 = 0x0F,
...
static int decode_key(list_t* kidx1, list_t* kidx2, list_t* kidx3, char* str, int str_len, int index) {
    ...
    if (i == str_len - 1) {
        ...
        i++; // make count (i=0 > len = 1)
    }

    str[i] = '\0'; // null terminator after count
    return i;
}
...
static bool get_expected_node(const psb_node_t* node, const char* key, psb_node_t* p_out, psb_type_t expected) {
