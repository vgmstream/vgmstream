...
static bool parse_header(fsb5_header_t* fsb, STREAMFILE* sf);
static void get_name(char* buf, size_t buf_size, int target_subsong, fsb5_header_t* fsb5, STREAMFILE* sf_fsb);
static layered_layout_data* build_layered_fsb5(STREAMFILE* sf, STREAMFILE* sb, fsb5_header_t* fsb5);
static void read_vorbis_seek(VGMSTREAM* v, STREAMFILE* sf, fsb5_header_t* fsb5);

/* FSB5 - Firelight's FMOD Studio SoundBank format */
VGMSTREAM* init_vgmstream_fsb5(STREAMFILE* sf) {
    VGMSTREAM* vgmstream = NULL;
    STREAMFILE* sb = NULL;
    fsb5_header_t fsb5 = {0};
    ...
    get_name(vgmstream->stream_name, STREAM_NAME_SIZE, fsb5.target_subsong, &fsb5, sf);
...
static bool parse_header(fsb5_header_t* fsb5, STREAMFILE* sf) {
    fsb5->target_subsong = sf->stream_index;
    ...
    if (!check_subsongs(&fsb5->target_subsong, fsb5->total_subsongs))
        return false;
    ...
    uint32_t offset = fsb5->base_header_size;
    for (int i = 0; i < fsb5->total_subsongs; i++) {
        uint32_t stream_header_size = 0;
        uint32_t data_offset = 0;

        uint64_t sample_mode = read_u64le(offset+0x00,sf);
        ...
        if (sample_mode & 0x01) { /* bits: 0 (1) */
            ...
            uint32_t extraflag_max = fsb5->base_header_size + fsb5->sample_header_size;

            while (extraflag_offset < extraflag_max) {
                uint32_t extraflag = read_u32le(extraflag_offset,sf);

                uint32_t extraflag_type = (extraflag >> 25) & 0x7F; /* bits 32..26 (7) */
                uint32_t extraflag_size = (extraflag >> 1) & 0xFFFFFF; /* bits 25..1 (24)*/
                uint32_t extraflag_continue = (extraflag & 0x01); /* bit 0 (1) */

                /* parse target only, as flags change between subsongs */
                if (i + 1 == fsb5->target_subsong) {
                    switch(extraflag_type) {
                        ...
                        case 0x03:  /* loop info */
                            ...
                            /* autodetect unwanted loops */
                            {
                                ...
                                bool full_loop = fsb5->loop_start == 0 && fsb5->loop_end + 1152 >= fsb5->num_samples; /* around ~15 samples less, ~1000 for MPEG */
                                /* a few longer Sonic songs shouldn't repeat */
                                bool is_small = true; //fsb5->num_samples < 20 * fsb5->sample_rate;

                                /* wrong values in some files [Pac-Man CE2 Plus (Switch) pce2p_bgm_ajurika_*.fsb] */
                                bool ajurika_loops = fsb5->loop_start == 0x3c && fsb5->loop_end == (0x007F007F + 1) &&
        ...
        if (i + 1 == fsb5->target_subsong) {
            ...
            if (i + 1 == fsb5->total_subsongs) {
                next_data_offset = fsb5->sample_data_size;
            }
            else {
                uint64_t next_sample_mode = read_u64le(offset+stream_header_size+0x00,sf);
    ...
    if (fsb5->name_table_size) {
        uint32_t name_suboffset = fsb5->base_header_size + fsb5->sample_header_size + 0x04*(fsb5->target_subsong-1);
