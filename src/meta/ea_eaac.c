...
static VGMSTREAM* init_vgmstream_eaaudiocore_header(STREAMFILE* sf_head, STREAMFILE* sf_data, off_t header_offset, off_t start_offset, meta_t meta_type, bool standalone) {
    ...
    if (meta_type == meta_EA_SPS) {
        header_block_id = read_u8(header_offset, sf_head);
        header_block_size = read_u32be(header_offset, sf_head) & 0x00FFFFFF;
    ...
    /* EA SNR/SPH header */
    header1 = read_u32be(header_offset + 0x00, sf_head);
    header2 = read_u32be(header_offset + 0x04, sf_head);
