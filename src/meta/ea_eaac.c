...
static size_t calculate_eaac_size(STREAMFILE *streamFile, eaac_header *ea, uint32_t num_samples, off_t start_offset, int is_ram);
    ...
    if (ea->version == EAAC_VERSION_V0) {
        switch (ea->type) {
            case EAAC_TYPE_RAM:
                ...
                data_size = calculate_eaac_size(streamHead, ea, ea->num_samples, ea->stream_offset, 1);
            ...
            case EAAC_TYPE_STREAM:
                ...
                data_size = calculate_eaac_size(streamData, ea, ea->num_samples, ea->stream_offset, 0);
                if (data_size == 0) goto fail;
            ...
            case EAAC_TYPE_GIGASAMPLE:
                ...
                data_size = calculate_eaac_size(streamHead, ea, ea->prefetch_samples, ea->prefetch_offset, 1);
                if (data_size == 0) goto fail;
                ...
                /* open main data */
                data_size = calculate_eaac_size(streamData, ea, ea->num_samples - ea->prefetch_samples, ea->stream_offset, 0);
                if (data_size == 0) goto fail;
        ...
        data_size = calculate_eaac_size(streamHead, ea, ea->num_samples, ea->stream_offset, ea->type == EAAC_TYPE_RAM);
        if (data_size == 0) goto fail;
