...
#define xbb_has_smpl_chunk    0x10
#define xbb_has_external_data 0x20
...
} xbb_riff_chunk_info; // RIFF chunks and helper for such.
...
} xbb_riff_wave_fmt_chunk_info; // RIFF WAVE format header.
...
} xbb_riff_wave_data_chunk_info; // RIFF WAVE data header, in case some sound entry needs an XSB file to be played back in-game.

/*
typedef struct {
    uint32_t loop_start;
    uint32_t loop_end;
} xbb_riff_wave_smpl_chunk_info; // RIFF WAVE "sample" header, in case some sound entry has to have some sort of loop metadata in it.
*/
...
typedef struct {
    ...
    /* RIFF chunks */
    xbb_riff_chunk_info riff;

    /* RIFF WAVE format header */
    xbb_riff_wave_fmt_chunk_info fmt;

    /* RIFF WAVE data header, in case some sound entry needs an XSB file to be played back in-game */
    xbb_riff_wave_data_chunk_info data;

    /* RIFF WAVE "sample" header, in case some sound entry has to have some sort of loop metadata in it */
    //xbb_riff_wave_smpl_chunk_info smpl;
...
VGMSTREAM* init_vgmstream_xbb_xsb(STREAMFILE* sf)
{
    ...
    for (i = 0; i < xbb.entries; i++)
    {
        ...
        if (xbb.is_riff == 0x46464952) { // "RIFF"
            ...
            xbb.riff.riff_size = read_u32le(xbb.entry_offset + 4, sf_h);
            xbb.entry_offset += 8;
            xbb.riff.riff_offset = xbb.entry_offset;
            xbb.entry_offset += xbb.riff.riff_size;
        } else {
            goto fail;
        }

        /* check if "RIFF" chunk flag exists, unlikely. */
        if (!(xbb.xbb_flags & xbb_has_riff_chunk)) goto fail;

        xbb.riff_size_from_zero = 0;
        /* read "WAVE" chunk */
        xbb.is_wave = read_u32le(xbb.riff.riff_offset + 0, sf_h);
        if (xbb.is_wave == 0x45564157) { // "WAVE"
            xbb.xbb_flags |= xbb_has_wave_chunk;
            xbb.riff_size_from_zero += 4;
            xbb.riff.riff_offset += 4;
        } else {
            goto fail;
        }
        ...
        while (xbb.riff_size_from_zero < xbb.riff.riff_size) {
            /* read the rest of the chunks */
            xbb.riff.chunk_type = read_u32le(xbb.riff.riff_offset + 0, sf_h);
            xbb.riff.chunk_size = read_u32le(xbb.riff.riff_offset + 4, sf_h);
            xbb.riff.riff_offset += 8;
            xbb.riff_size_from_zero += 8;
            switch (xbb.riff.chunk_type) {
                case 0x20746d66: // "fmt "
                    xbb.xbb_flags |= xbb_has_fmt_chunk;
                    xbb.riff.fmt_offset = xbb.riff.riff_offset;
                    xbb.riff.fmt_size = xbb.riff.chunk_size;
                    xbb.riff.riff_offset += xbb.riff.fmt_size;
                    xbb.riff_size_from_zero += xbb.riff.fmt_size;
                    break;
                case 0x61746164: // "data"
                    xbb.riff.data_offset = xbb.riff.riff_offset;
                    xbb.xbb_flags |= xbb_has_data_chunk;
                    if (xbb.riff.chunk_size == 0xfffffff8) {
                        xbb.xbb_flags |= xbb_has_external_data;
                        xbb.riff.riff_offset += 8;
                        xbb.riff_size_from_zero += 8;
                    }
                    else {
                        xbb.riff.data_size = xbb.riff.chunk_size;
                        xbb.riff.riff_offset += xbb.riff.data_size;
                        xbb.riff_size_from_zero += xbb.riff.data_size;
                    }
                    break;
                case 0x6c706d73: // "smpl"
                    xbb.xbb_flags |= xbb_has_smpl_chunk;
                    xbb.riff.smpl_offset = xbb.riff.riff_offset;
                    xbb.riff.smpl_size = xbb.riff.chunk_size;
                    xbb.riff.riff_offset += xbb.riff.smpl_size;
                    xbb.riff_size_from_zero += xbb.riff.smpl_size;
                    break;
                default:
                    break;
            }
        ...
        if (i + 1 == target_subsong) {
            ...
            if (xbb.riff.fmt_size == 0x14) {
                /* Xbox IMA ADPCM codec header data */
                xbb.fmt.codec = read_u16le(xbb.riff.fmt_offset + 0, sf_h);
                xbb.fmt.channels = read_u16le(xbb.riff.fmt_offset + 2, sf_h);
                xbb.fmt.sample_rate = read_u32le(xbb.riff.fmt_offset + 4, sf_h);
                xbb.fmt.avg_bps = read_u32le(xbb.riff.fmt_offset + 8, sf_h);
                xbb.fmt.blkalgn = read_u16le(xbb.riff.fmt_offset + 12, sf_h);
                xbb.fmt.bit_depth = read_u16le(xbb.riff.fmt_offset + 14, sf_h);
                xbb.fmt.extra_size = read_u16le(xbb.riff.fmt_offset + 16, sf_h);
                if (xbb.fmt.extra_size > 0) {
                    xbb.fmt.blksmpl = read_u16le(xbb.riff.fmt_offset + 18, sf_h);
                }
                /* do some checks against existing fmt values */
                if (xbb.fmt.blkalgn != (0x24 * xbb.fmt.channels)) goto fail;
                if (xbb.fmt.bit_depth != 4) goto fail;
                if (xbb.fmt.extra_size != 2) goto fail;
                if (xbb.fmt.blksmpl != 0x40) goto fail;
            } else {
                /* no known fmt sizes. */
                vgm_logi("XBB+XSB: unknown fmt size %d\n", xbb.riff.fmt_size);
                goto fail;
            }
            /* check if "smpl" chunk exists, although it's not used by the game files themselves. */
            if ((xbb.xbb_flags & xbb_has_smpl_chunk)) {
                /* catch the "smpl is present" flag just in case we happen to find an "smpl" chunk. */
                vgm_logi("XBB+XSB: found smpl chunk\n");
                goto fail;
            }
            break;
        }
    ...
    if (i + 1 == target_subsong) {
        ...
        if (xbb.xbb_flags & xbb_has_external_data) {
            ...
            xbb.data.external_data_offset = read_u32le(xbb.riff.data_offset + 0, sf_h);
            xbb.data.external_data_size = read_u32le(xbb.riff.data_offset + 4, sf_h);
            xbb.stream_offset = xbb.data.external_data_offset;
            xbb.stream_size = xbb.data.external_data_size;
            ...
            xbb.stream_offset = xbb.riff.data_offset;
            xbb.stream_size = xbb.riff.data_size;
            sf_data = sf_h;
        }
    }

    /* build and allocate the vgmstream */
    vgmstream = allocate_vgmstream(xbb.fmt.channels, 0);
    if (!vgmstream) goto fail;

    vgmstream->meta_type = meta_XBB_XSB;
    vgmstream->sample_rate = xbb.fmt.sample_rate;
    vgmstream->num_streams = xbb.entries;
    vgmstream->stream_size = xbb.stream_size;

    switch (xbb.fmt.codec) {
        case 0x0069:
            vgmstream->coding_type = coding_XBOX_IMA;
            vgmstream->layout_type = layout_none;
            vgmstream->num_samples = xbox_ima_bytes_to_samples(xbb.stream_size,xbb.fmt.channels);
            break;
        default:
            vgm_logi("XBB+XSB: unknown codec 0x%04x\n", xbb.fmt.codec);
            goto fail;
    }
