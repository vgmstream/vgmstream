...
{ 
    ...
    int32_t id; /* sound_id */ 
    int32_t realsize; // real_sound_data_size; didn't exist in Spider-Man PS2 snd format. 
    int32_t size; /* sound_data_size */ 
    int32_t offset; /* sound_offset */ 
    int32_t freq; // sample_rate; present in snd format only. 
    uint16_t pitch; // manually calc'd from snd format; sdx format has it built-in. 
    uint16_t volume_l; // ditto. 
    uint16_t volume_r; // ditto. 
    int32_t source; // sound_source, for a grand total of 7 (within the 0-6 range) 
    uint32_t flags; /* sound_flags */ 
    int32_t lang; // sound_language, for a grand total of 12 (within the 0-11 range) 
    // sdx format introduced two vars, here they are for reference sake. 
    uint32_t unk0x48; // alternates between 0x5000 and a literal zero, with no in-between. 
    //uint32_t unk0x54; // *always* zero. 
...
VGMSTREAM* init_vgmstream_snd_vbc(STREAMFILE* sf) { 
    ...
    /* checks (file extension, file size, and a score-baed "is it really a text file?" check; in that order) */ 
    if (!check_extensions(sf, "snd")) 
        goto fail; 
 
    if (get_streamfile_size(sf) < 0x40 && get_streamfile_size(sf) > 0x17000) 
        goto fail; 
...
VGMSTREAM* init_vgmstream_sdx_vbc(STREAMFILE* sf) { 
    ...
    vgmstream->sample_rate = round10((48000 * ps2snd.selected_entry.pitch) / 4096); 
    // ^ alternatively, (ps2snd.selected_entry.pitch * 48000) << 12 
    vgmstream->num_streams = ps2snd.total_subsongs; 
    vgmstream->stream_size = ps2snd.selected_entry.size; 
...
static int parse_line_commands(STREAMFILE* sf, treyarch_ps2snd* ps2snd, bool register_subsong); 
...
static int parse_treyarch_ps2_snd(treyarch_ps2snd* ps2snd) { 
    ...
    // initialize subsong info. 
    ps2snd->total_subsongs = 0; 
    if (ps2snd->target_subsong == 0) 
        ps2snd->target_subsong = 1; 
    ...
    { 
        ...
        if (!parse_line_commands(ps2snd->sf_header, ps2snd, false)) 
            break; 
    ...
    // now read the whole file again so we can choose a subsong for the vgmstream to make do with. 
    int got_subsong = 0; 
    do 
    { 
        line_len = text_reader_get_line(&tr, &ps2snd->line); 
        ...
        got_subsong = parse_line_commands(ps2snd->sf_header, ps2snd, true); 
        if (!got_subsong) 
            return 0; 
...
// parses line commands (tokens) from an SND text line. 
// register_subsong is what controls the following circumstances: 
// 1. if/when total subsongs should be counted 
// 2. if/when vgmstream needs a subsong 
//  
// on the places where register_subsong is set to false, 
// this function will simply count as many subsongs as the SND permits. 
//  
// but on the places where register_subsong is set to true, 
// this function will find an "appropriate suitor" 
// for when vgmstream is requesting a subsong number from this snd_vbc meta. 
// this requires that ALL subsongs be counted from an SND file beforehand, 
// which is made possible by setting the third function arg to false. 
//  
// only "proper" way to do this would be: 
// 1. call the function once, and set third arg to false while at it. 
// 2. call it twice, this time set third arg to true. 
static int parse_line_commands(STREAMFILE* sf, treyarch_ps2snd* ps2snd, bool register_subsong) { 
    ...
    // boundary checks for line position that goes beyond what's actually in the line itself. 
    ...
    while (ps2snd->entry.cmds = get_string_cmd_key(&state, ps2snd->line, &command, &int_value, char_array_small))
    {
        ...
        case CD: /* STREAM.VBC flag */ 
            ...
            break; 
        case INT: /* int number. */ 
            if (!defined_sound_name_or_id) // do nothing with it and proceed as if nothing happened. 
                VGM_LOG("SND+VBC: Skipping this int (%d) \n", int_value); 
            if (defined_sound_name_or_id) // sound name/ID was already parsed beforehand. 
                defined_sound_name_or_id = false; 
            break; 
        case STRING: /* string. */ 
            if (!defined_sound_name_or_id) // do nothing with it and proceed as if nothing happened. 
                VGM_LOG("SND+VBC: Skipping this string ('%s') \n", char_array_small); 
            if (defined_sound_name_or_id) // sound name/ID was already parsed beforehand. 
                defined_sound_name_or_id = false; 
            break; 
    ...
    if (!(ps2snd->entry.flags >> 4 & 1)) 
        ps2snd->entry.unk0x48 = 0x5000; 
 
    if (register_subsong) 
        if (ps2snd->target_subsong < 0 || ps2snd->target_subsong > ps2snd->total_subsongs || ps2snd->target_subsong > 1000 || ps2snd->total_subsongs < 1) 
            return 0; 
    // observed max number of subsongs for an snd text file is 960. [Spider-Man (2002) (PS2)] 
 
    if (ps2snd->sound_has_defined_name_or_id && register_subsong) { 
    ...
    { 
        ...
        { 
            ...
            { 
                ...
                if (val_chr == '\0') break; 
                if (val_chr >= 'a' && val_chr <= 'z') val_chr ^= 0x20; 
            ...
            *out_val1 = 0; 
            ...
            if (has_letters == 0 && has_numbers != 0) 
                stream_name_is_not_all_numbers = false; 
            else if (has_letters != 0 && has_numbers == 0) 
                stream_name_is_not_all_numbers = true; 
            else if (has_letters != 0 && has_numbers != 0) 
                stream_name_is_not_all_numbers = true; 
            ...
            { 
                ...
                if (strcmp(cfg->token_str, "SIZE") == 0) 
                    *out_val1 = SIZE; 
                else if (strcmp(cfg->token_str, "REALSIZE") == 0) 
                    *out_val1 = REALSIZE; 
                else if (strcmp(cfg->token_str, "OFFSET") == 0) 
                    *out_val1 = OFFSET; 
                else if (strcmp(cfg->token_str, "FREQ") == 0) 
                    *out_val1 = FREQ; 
                else if (strcmp(cfg->token_str, "LOOP") == 0) 
                    *out_val1 = LOOP; 
                else if (strcmp(cfg->token_str, "REVERB") == 0) 
                    *out_val1 = REVERB; 
                else if (strcmp(cfg->token_str, "VOLL") == 0) 
                    *out_val1 = VOLUME_L; 
                else if (strcmp(cfg->token_str, "VOLR") == 0) 
                    *out_val1 = VOLUME_R; 
                else if (strcmp(cfg->token_str, "VOLUME") == 0) 
                    *out_val1 = VOLUME; 
                else if (strcmp(cfg->token_str, "STEREO") == 0) 
                    *out_val1 = STEREO; 
                else if (strcmp(cfg->token_str, "MONO") == 0) 
                    *out_val1 = MONO; 
                else if (strcmp(cfg->token_str, "SPU") == 0) 
                    *out_val1 = SPU; 
                else if (strcmp(cfg->token_str, "COLLECTION") == 0) 
                    *out_val1 = COLLECTION; 
                else if (strcmp(cfg->token_str, "CD") == 0) 
                    *out_val1 = CD; 
                else { 
                    int strcmp_return = 0; 
                    int snd_lang_i = 0; 
                    ...
                    for (; strcmp_return = strcmp(treyarch_ps2_sound_languages[snd_lang_i], "N\\A"); strcmp_return != 0) 
                    { 
                    ...
                    // (todo) generates C4552 warning. 
                    for (; strcmp_return = strcmp(treyarch_ps2_sound_sources[snd_src_i], "N\\A"); strcmp_return != 0) 
                    { 
        ...
        /* if token already exists, just copy info from the token and reset everything. */ 
        ...
        { 
            ...
            { 
                ...
                { 
                    ...
                    // (todo) current code not nice with non-ASCII pathnames. 
                    ...
                    { 
                        ...
                        { 
                            if (((char_comp1 & 0x80) == 0) && ((char_comp1 & 0x40) == 0x40) && ((char_comp1 & 0x20) == 0x20)) 
                                char_comp1 ^= 0x20; 
                            if (((char_comp2 & 0x80) == 0) && ((char_comp2 & 0x40) == 0x40) && ((char_comp2 & 0x20) == 0x20)) 
                                char_comp2 ^= 0x20; 
                        } 
...
static int parse_sdx_binary_struct(STREAMFILE* sf, treyarch_ps2snd* ps2snd, sdx_bin_reader_state* cfg, bool register_subsong); 
...
static int parse_sdx(treyarch_ps2snd* ps2snd) { 
    ...
    if (ps2snd->header_size < ps2snd->sdx_block_info_size) 
        return 0; 
 
    /* step 2 - if one value isn't zero and other value isn't "small enough"(*), might as well just fail. */ 
    if ((read_u32le(0x30, ps2snd->sf_header)) != 0 
        && 
        (read_u32le(0x34, ps2snd->sf_header)) > 0x10000) 
        return 0; 
...
static int parse_sdx_binary_struct(STREAMFILE* sf, treyarch_ps2snd* ps2snd, sdx_bin_reader_state* cfg, bool register_subsong) { 
    ...
    // if we're not counting the subsongs, 
    // we're not checking if pos is below or above what SDX weighs in bytes. 
    if (!register_subsong) 
    { 
        if (cfg->bin_pos < 0) 
            return 0; 
