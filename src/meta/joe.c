...
VGMSTREAM* init_vgmstream_joe(STREAMFILE* sf) {
    ...
    if (data_size == file_size - 0x800
            && unknown1 == 0x00002000 && unknown2 == 0xFFFFFFFF) {
        /* NYR (PS2) */
        interleave = 0x2000;
        start_offset = 0x800;
    }
    else if (data_size / 2 == file_size - 0x10
            && unknown1 == 0x0045039A && unknown2 == 0x00108920) {
        /* Super Farm (PS2) */
        ...
        start_offset = 0x10;
            ...
            && unknown1 == 0xCCCCCCCC && unknown2 == 0xCCCCCCCC) {
        /* Sitting Ducks (PS2) */
        data_size = data_size / 2;
        interleave = 0x8000;
        start_offset = 0x10;
            ...
            && unknown1 == 0xCCCCCCCC && unknown2 == 0xCCCCCCCC) {
        /* The Mummy: The Animated Series (PS2) */
        interleave = 0x8000;
        start_offset = 0x10;
    }
    else if (data_size == file_size - 0x4020) {
        /* Counter Terrorism Special Forces (PS2), all games beyond */
