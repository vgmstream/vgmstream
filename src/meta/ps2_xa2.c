...
/* XA2 (2 Channels) (XG3 Extreme-G Racing) */
VGMSTREAM * init_vgmstream_ps2_xa2(STREAMFILE *streamFile) {
	...
	/* check header */
    if (read_32bitBE(0x05,streamFile) != 0x100000)
        goto fail;
...
/* XA2 (4 Channels) (XG3 Extreme-G Racing) */
VGMSTREAM * init_vgmstream_ps2_xa2_xg3(STREAMFILE *streamFile) {
    VGMSTREAM * vgmstream = NULL;
    char filename[260];
    off_t start_offset;
    ...
    vgmstream->layout_type = layout_interleave;
    vgmstream->interleave_block_size = read_32bitLE(0x04,streamFile);
	vgmstream->meta_type = meta_PS2_XA2_XG3;
