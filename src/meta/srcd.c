...
VGMSTREAM* init_vgmstream_srcd(STREAMFILE* sf) {
...

    {
        ...
        if (loop_flag && (loop_start_sample < 0 || loop_end_sample < 0 || loop_start_sample >= loop_end_sample)) {
            VGM_LOG("SRCD: Invalid loop points found (start=%i, end=%i), disabling loop\n", loop_start_sample, loop_end_sample);
            loop_flag = 0;
        }
    }
