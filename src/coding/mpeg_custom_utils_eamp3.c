...
typedef struct {
    ...
    uint32_t skip_size; /* size of the header part */
...
static bool eamp3_write_pcm_block(VGMSTREAMCHANNEL* stream, mpeg_codec_data* data, int num_stream, eamp3_frame_info* eaf);
static bool eamp3_skip_data(VGMSTREAMCHANNEL* stream, mpeg_codec_data* data, int num_stream, int at_start);
...
int mpeg_custom_setup_init_eamp3(STREAMFILE* sf, off_t start_offset, mpeg_codec_data *data, coding_t *coding_type) {
    ...
    if (!eaf.mpeg_size) {
        ...
        *coding_type = coding_MPEG_layer3;
        ...
        if (!mpeg_get_frame_info(sf, start_offset + eaf.skip_size, &info))
            return false;
        switch(info.layer) {
...
int mpeg_custom_parse_frame_eamp3(VGMSTREAMCHANNEL *stream, mpeg_codec_data *data, int num_stream) {
    ...
    ms->bytes_in_buffer = read_streamfile(ms->buffer, stream->offset + eaf.skip_size, eaf.mpeg_size, stream->streamfile);
...
static bool eamp3_parse_frame(STREAMFILE* sf, uint32_t offset, mpeg_codec_data* data, eamp3_frame_info* eaf) {
    ...
    if (eaf->extended_flag > 0) {
        ...
        eaf->skip_size  = 0x02 + 0x04;

        eaf->pcm_size   = sizeof(short) * eaf->pcm_number * data->channels_per_frame;
        eaf->mpeg_size  = eaf->frame_size - eaf->skip_size - eaf->pcm_size;
        if (eaf->frame_size < eaf->skip_size + eaf->pcm_size) {
            VGM_LOG("EAMP3: bad pcm size at %x: %x < %x + %x (%x * %x)\n", offset, eaf->frame_size, eaf->skip_size, eaf->pcm_size, eaf->pcm_number, data->channels_per_frame);
            return false;
        }
    }
    else {
        ...
        eaf->pcm_size   = 0;
        eaf->mpeg_size  = eaf->frame_size - eaf->skip_size;
    }
...
static bool eamp3_write_pcm_block(VGMSTREAMCHANNEL* stream, mpeg_codec_data* data, int num_stream, eamp3_frame_info* eaf) {
    ...
    if (ms->samples_filled + eaf->pcm_number > max_samples) {
        VGM_LOG("EAMP3: can't fill the sample buffer with %i samples\n", eaf->pcm_number);
        return false;
    }
    ...
    off_t pcm_offset = stream->offset + eaf->skip_size + eaf->mpeg_size;
