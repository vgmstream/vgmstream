...
macro(setup_target TARGET)
	...
	if(USE_MPEG)
		target_compile_definitions(${TARGET} PUBLIC VGM_USE_MPEG)
		if(WIN32)
			if(LINK)
				add_dependencies(${TARGET} libmpg123)
				target_link_libraries(${TARGET} PUBLIC ${VGM_BINARY_DIR}/ext_libs/libmpg123-0.lib)
			endif()
		else()
			target_include_directories(${TARGET} PRIVATE ${MPG123_INCLUDE_DIR})
			if(LINK)
				target_link_libraries(${TARGET} PUBLIC mpg123 m)
			endif()
		endif()
	endif()

	if(USE_VORBIS)
		target_compile_definitions(${TARGET} PUBLIC VGM_USE_VORBIS)
		if(WIN32)
			if(LINK)
				add_dependencies(${TARGET} libvorbis)
				target_link_libraries(${TARGET} PUBLIC ${VGM_BINARY_DIR}/ext_libs/libvorbis.lib)
			endif()
		else()
			target_include_directories(${TARGET} PRIVATE ${VORBISFILE_INCLUDE_DIRS})
			if(LINK)
				target_link_libraries(${TARGET} PUBLIC vorbisfile vorbis ogg m)
			endif()
		endif()
	endif()

	if(USE_FFMPEG)
		target_compile_definitions(${TARGET} PUBLIC VGM_USE_FFMPEG)
			...
			if(LINK)
				if(FFMPEG_COMPILE)
					if(USE_FFMPEG_LIBOPUS)
						target_link_libraries(${TARGET} PUBLIC avformat avcodec avutil swresample opus pthread m)
					else()
						target_link_libraries(${TARGET} PUBLIC avformat avcodec avutil swresample pthread m)
					endif()
				else()
	...
	if(USE_G7221)
		target_compile_definitions(${TARGET} PUBLIC VGM_USE_G7221)
	endif()

	if(USE_G719)
		target_compile_definitions(${TARGET} PUBLIC VGM_USE_G719)
		if(WIN32 AND LINK)
			add_dependencies(${TARGET} libg719_decode)
			target_link_libraries(${TARGET} PUBLIC ${VGM_BINARY_DIR}/ext_libs/libg719_decode.lib)
		endif()
		if(NOT WIN32 AND LINK)
			target_link_libraries(${TARGET} PUBLIC g719_decode)
		endif()
	endif()

	if(USE_ATRAC9)
		target_compile_definitions(${TARGET} PUBLIC VGM_USE_ATRAC9)
		if(WIN32 AND LINK)
	...
	if(USE_CELT)
		target_compile_definitions(${TARGET} PUBLIC VGM_USE_CELT)
		if(WIN32 AND LINK)
		...
		if(NOT WIN32 AND LINK)
			target_link_libraries(${TARGET} PRIVATE celt0061 celt0110 m)
		endif()
	endif()

    if(USE_SPEEX)
        target_compile_definitions(${TARGET} PUBLIC VGM_USE_SPEEX)
        if(WIN32 AND LINK)
            add_dependencies(${TARGET} libspeex)
			target_link_libraries(${TARGET} PUBLIC ${VGM_BINARY_DIR}/ext_libs/libspeex-1.lib)
        endif()
		if(NOT WIN32 AND LINK)
			target_link_libraries(${TARGET} PUBLIC speex m)
		endif()
    endif()
endmacro()
