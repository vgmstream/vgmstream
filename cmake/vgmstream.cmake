...
macro(setup_target TARGET)
	...
	if(USE_MPEG)
		...
		if(WIN32)
			if(LINK)
				...
				target_link_libraries(${TARGET} PRIVATE ${VGM_BINARY_DIR}/ext_libs/libmpg123-0.lib)
			endif()
		else()
			target_include_directories(${TARGET} PRIVATE ${MPG123_INCLUDE_DIR})
			if(LINK)
				target_link_libraries(${TARGET} PRIVATE mpg123 m)
			endif()
		endif()
	endif()
	...
	if(USE_VORBIS)
		...
		if(WIN32)
			if(LINK)
				...
				target_link_libraries(${TARGET} PRIVATE ${VGM_BINARY_DIR}/ext_libs/libvorbis.lib)
			endif()
		else()
			target_include_directories(${TARGET} PRIVATE ${VORBISFILE_INCLUDE_DIRS})
			if(LINK)
				target_link_libraries(${TARGET} PRIVATE vorbisfile vorbis ogg m)
			endif()
		endif()
	endif()
			...
			if(LINK)
				if(FFMPEG_COMPILE)
					if(USE_FFMPEG_LIBOPUS)
						target_link_libraries(${TARGET} PRIVATE avformat avcodec avutil swresample opus pthread m)
					else()
						target_link_libraries(${TARGET} PRIVATE avformat avcodec avutil swresample pthread m)
					endif()
				else()
	...
	if(USE_G719)
		...
		if(WIN32 AND LINK)
			...
			target_link_libraries(${TARGET} PRIVATE ${VGM_BINARY_DIR}/ext_libs/libg719_decode.lib)
		endif()
		if(NOT WIN32 AND LINK)
			target_link_libraries(${TARGET} PRIVATE g719_decode)
		endif()
	endif()
    ...
    if(USE_SPEEX)
        ...
        if(WIN32 AND LINK)
			...
			target_link_libraries(${TARGET} PRIVATE ${VGM_BINARY_DIR}/ext_libs/libspeex-1.lib)
        endif()
		if(NOT WIN32 AND LINK)
			target_link_libraries(${TARGET} PRIVATE speex m)
		endif()
    endif()
endmacro()
